...
class RandomForestURLModel(BaseClassifierModel):
    """Random‑Forest ensemble URL classifier."""

    def __init__(self, **clf_kwargs):
        self.vectorizer = TfidfVectorizer(
            analyzer="char", ngram_range=(3, 5), max_features=50_000
        )
        default_params = {'n_estimators': 200, 'max_depth': 30, 'min_samples_leaf': 3, 'n_jobs': -1, 'random_state': 42}
    ...
    def train_and_plot(self, X: Sequence[str], y: Sequence[str], *, save_dir: Path) -> Path:
        ...
        plt.tight_layout()
        png_path = save_dir / "random_forest_confusion_matrix.png"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train Random Forest URL model")
    parser.add_argument(
        "--csv",
        type=str,
